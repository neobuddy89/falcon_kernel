...
# Needed for module compilation
mkdir -p $KERNELDIR/out/system/lib/modules/pronto
...
mv $KERNELDIR/out/system/lib/modules/wlan.ko $KERNELDIR/out/system/lib/modules/pronto/pronto_wlan.ko
cd $KERNELDIR/out/system/lib/modules;
ln -sf pronto/pronto_wlan.ko wlan.ko;
cd $KERNELDIR;
chmod -R 755 $KERNELDIR/out/system/lib/modules;
rm -rf $KERNELDIR/out/tmp_modules >> /dev/null;
...
if [ -e $KERNELDIR/arch/arm/boot/zImage ]; then
	...
	./utilities/mkbootimg --kernel zImage --ramdisk ramdisk.gz --cmdline "ttyHSL0,115200,n8 androidboot.console=ttyHSL0 androidboot.hardware=qcom user_debug=31 msm_rtb.filter=0x37 vmalloc=400M utags.blkdev=/dev/block/platform/msm_sdcc.1/by-name/utags androidboot.write_protect=0 zcache" --base 0x00000000 --pagesize 2048 --ramdisk_offset 0x01000000 --tags_offset 0x00000100 --dt dt.img --output boot.img
